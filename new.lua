...
function Library:KeybindList()
    ...
    function KeyListObj:UpdateItem(name, state, keyText)
        ...
        if state and validKey then
            if not KeyListObj.Items[name] then
                ...
                table.insert(Library.Elements.Labels, NameLabel)
            ...
            if KeyListObj.Items[name] then
                KeyListObj.Items[name].Frame.Visible = false
            end
        end
    end
    ...
    KeyListObj.List = ListHolder

    function KeyListObj:SetVisibility(v) Frame.Visible = (v == true) end
    Library.KeyList = KeyListObj
    return KeyListObj
end
    ...
    function Window:Tab(name)
        ...
        function Tab:Section(sectionName, side, subObj)
            ...
            function Section:Button(opts)
                ...
                local function updateState(state)
                    ...
                    if isActive then
                        ...
                        Label.Text = string.gsub(Label.Text, "%s*%[ON%]", "")
                    end
                end
                local btnObj = { Holder = Holder }
                function btnObj:SetText(t) local str = (type(self) == "table") and t or self; Label.Text = tostring(str or "") end
                btnObj.set = function(self, t) local str = (type(self) == "table") and t or self; Label.Text = tostring(str or "") end
                btnObj.Set = btnObj.set
                function btnObj:SetState(s) local state = (type(self) == "table") and s or self; updateState(state) end
                function btnObj:SetVisibility(s) local state = (type(self) == "table") and s or self; Holder.Visible = (state == true) end
                return btnObj
            end
            ...
            function Section:Label(opts)
                ...
                local Label = Create("TextLabel", { Text = text, Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, FontFace = true, TextSize = Library.Config.FontSize, TextColor3 = Library.Theme.Text, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 7, Parent = Holder, TextStrokeTransparency = 0, TextStrokeColor3 = Color3.fromRGB(0,0,0), RichText = true })
                table.insert(Library.Elements.Labels, Label)
                local Obj = { Holder = Holder }
                function Obj:SetText(t) local str = (type(self) == "table") and t or self; Label.Text = tostring(str or "") end
                function Obj:set(t) local str = (type(self) == "table") and t or self; Label.Text = tostring(str or "") end
                function Obj:Set(t) local str = (type(self) == "table") and t or self; Label.Text = tostring(str or "") end
                function Obj:SetVisibility(s) local state = (type(self) == "table") and s or self; Holder.Visible = (state == true) end
